...
def configHotspotWifi(msg):
    global hotspot
    try:
       #os.system("sudo nmcli con down Hotspot")
...
cmd = {
    ...
    "LI" : "nexus.writeBytes(':LI#',array_to_bytes(patch))",
    "LP" : "nexus.writeBytes(':LP#',array_to_bytes(psfArray))",
    ...
    "SO" : "nexus.write(':SO'+wifiOnOff(msg.strip('#')[3:])+'#')"
}
